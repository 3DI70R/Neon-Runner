...
void app_main() 
{
    ...
    while(true) 
    {
        ...
        for(int i = 1; i < 16; i++) 
        {
            ...
            vector2 ut1 = transform_apply(vector2_add(s1.position, vector2_from_angle(s1.angle + DEGTORAD(90), s1.width + 3)), 1);
            vector2 ut2 = transform_apply(vector2_add(s2.position, vector2_from_angle(s2.angle + DEGTORAD(90), s2.width + 3)), 1);
            vector2 dt1 = transform_apply(vector2_add(s1.position, vector2_from_angle(s1.angle - DEGTORAD(90), s1.width + 3)), 1);
            vector2 dt2 = transform_apply(vector2_add(s2.position, vector2_from_angle(s2.angle - DEGTORAD(90), s2.width + 3)), 1);
            ...
            draw_set_buffer(KC_SCREEN_W, KC_SCREEN_H, background_buffer);
            draw_triangle(ut1.x, ut1.y, ut2.x, ut2.y, dt1.x, dt1.y, 0);
            draw_triangle(ut1.x, dt1.y, dt2.x, dt2.y, ut2.x, ut2.y, 0);
        }
